...
I have been examining a proposed scheme that asserts the following:
...
Maxwell’s equations in a passive, linear, lossless medium are time-reversal invariant in the standard sense. Optical phase conjugation and adaptive optics are established techniques: pre-distort a wavefront so free-space propagation partially undoes aberration. That does **not** require 4D geometry.

### 2.2 Superoscillations (Berry & Popescu and related work)

A band-limited function can, on a finite interval, oscillate faster than its highest Fourier component. That is real and experimentally demonstrated. It does not violate Maxwell: the spectrum remains band-limited.
...
$$
\frac{E_{\mathrm{core}}}{E_{\mathrm{total}}} \ll 1 \qquad\text{(often exponentially small in the “how super” parameter).}
$$

For a single photon, the Born rule identifies $\lvert \psi \rvert^2$ with detection probability. If the classical energy fraction in the core is $\varepsilon$, the single-shot probability of detection in the core is of order $\varepsilon$, not $\sim 1$.

I keep a toy numerical demonstration in:
...
**True:** A tesseract has **8 cubic cells** as 3D boundary facets. The hyperoctahedral / Coxeter group $B_4$ is a real symmetry group of the 4-cube ($\lvert B_4 \rvert=4!\cdot 2^4=384$ signed permutations of $\mathbb{R}^4$).

**Sealed count (optical phase space, first order):**  
On the paraxial phase space $(x,p_x,y,p_y)$ with symplectic form

$$
J=\mathrm{blockdiag}(J_2,J_2),\qquad J_2=\begin{pmatrix}0&1\\-1&0\end{pmatrix},
$$
...
Controllable degrees of freedom of a phase-only SLM: $N_{\mathrm{pix}}$ (e.g. $1920\times 1080$) times spectral bins if used — **hardware**, not 8/16/24/32 from the 4-cube.
...
This is the same **pattern of undeclared power** as in the archived script `quantum_information_cosmos.py`: take a clean mathematical object ($\ln 4$, $B_4$) and **declare** that it sets a physical frequency or optical control law without derivation.
...
1. Propagation is linear, band-limited (effective $\lvert k \rvert\le k_{\max}$).
2. Detection of one photon is described by a POVM / Born rule from a normalized field mode $\psi$.
3. The “sub-Airy core” is a region $C$ with $\int_C \lvert \psi \rvert^2 = \varepsilon \ll 1$ for any superoscillatory design that meaningfully beats the diffraction scale on $C$.
...
**Conclusion:** Imposing $B_4$ as a symmetry **restricts** first-order devices ($10\to 1$ free directions in the Lie algebra, up to that commutant). It cannot be a derivation of **extra** independent optical channels equal to the number of cells or vertices of the 4-cube. Any claim “8 cells → 8 channels” confuses **polytope combinatorics** with **operator rank**.
...
A clean PC-scale project (no ad hoc free-parameter tuning): run `scripts/superoscillation_energy_cost_demo.py` and report $E_{\mathrm{core}}/E_{\mathrm{total}}$ versus bandwidth — **confirming** the tax, not bypassing it.
...
I apply the same **methodological** standard: no elegant object ($\ln 4$, $B_4$, SDiff) gets observational power without a derived map to a measurable and an honest amplitude estimate.
