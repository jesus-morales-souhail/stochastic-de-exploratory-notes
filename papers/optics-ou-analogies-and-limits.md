...
**Repo \(\theta\), \(\sigma\):** OU kernel \(C(\Delta x)=\sigma_X^2 e^{-\theta|\Delta x|}\); MLE drives \(\sigma_X\to 0\), \(\theta\) poorly constrained near the boundary

---

I collected three recurring questions that sit at the border of optics and the OU residual model. For each I want a careful yes, a formal analogy, or a clear no — without mixing them.
...
- Ray optics in the paraxial regime: **ABCD** (or \(4\times 4\) for \(x,\theta_x,y,\theta_y\)) is standard and symplectic.
...
None of (1)–(3) is derived from Maxwell + a physical tesseract cavity in the text I have seen. A 4D hypercube has 8 cubic cells as a **mathematical boundary**; that does **not** automatically supply eight independent optical channels beyond a 2D phase mask.
...
$$
\mathrm{Cov}\big(X(x_i),X(x_j)\big) =\frac{\sigma^2}{2\theta}\, e^{-\theta|x_i-x_j|}.
$$
...
**Direction for this project:** I keep **scale + operator** discipline (BAO residuals, model kills, Option 0 slip with amplitude honesty). I do **not** merge tesseract optics into the DESI/SDiff papers.
...
$$
\frac{1}{s_o}+\frac{1}{s_i}-\frac{1}{f}=0,\qquad P=|\psi|^2.
$$

**OU (OK, repo):**

$$
\mathrm{d}X=-\theta X\,\mathrm{d}x+\sigma\,\mathrm{d}W,\qquad C_{ij}\propto e^{-\theta|x_i-x_j|}.
$$
